...
INTRAOP_SMOOTH_COVARS_DEFAULT = ["RRtotal", "TVinsp", "Pmean"]
_intraop_smooth_raw = os.getenv("INTRA5_INTRAOP_SMOOTH_COVARS", "").strip()
if _intraop_smooth_raw:
    if _intraop_smooth_raw.lower() in ("none", "null", "na"):
        INTRAOP_SMOOTH_COVARS = []
    else:
        INTRAOP_SMOOTH_COVARS = [c.strip() for c in _intraop_smooth_raw.split(",") if c.strip()]
else:
    INTRAOP_SMOOTH_COVARS = INTRAOP_SMOOTH_COVARS_DEFAULT.copy()
...
# Export slice variables (for curve output) without changing model's primary smooth vars.
# Default keeps old behavior (primary only). Use:
#   INTRA5_SLICE_VARS=all_intraop
# or INTRA5_SLICE_VARS=ET_CO2,TEMP,FiO2_new,MAP,SV,HR,RRtotal,TVinsp,Pmean,CI
SLICE_VARS_RAW = os.getenv("INTRA5_SLICE_VARS", "").strip()
...
def run_one_model(
    ...
    if not SLICE_VARS_RAW:
        slice_vars = primary_vars.copy()
    else:
        raw = SLICE_VARS_RAW.strip().lower()
        if raw == "all_intraop":
            preferred = primary_vars + [c for c in OPTIONAL_INTRAOP_COVARS if c not in primary_vars]
            slice_vars = [c for c in preferred if c in feature_names]
        else:
            req = [c.strip() for c in SLICE_VARS_RAW.split(",") if c.strip()]
            slice_vars = [c for c in req if c in feature_names]
            # Keep primary vars present to avoid accidental omission.
            for c in primary_vars:
                if c in feature_names and c not in slice_vars:
                    slice_vars.append(c)
    if not slice_vars:
        slice_vars = primary_vars.copy()

    x_grid_map = {}
    for xvar in slice_vars:
    ...
    slope_store = {xvar: [] for xvar in slice_vars} if need_slope_outputs else {}
    metrics_store = {xvar: [] for xvar in slice_vars} if EXPORT_SLOPE_METRICS else {}
    ...
    for xvar in slice_vars:
        ...
        if xvar in primary_vars:
            ...
            # For non-primary covariates, use reference-slice curve as adjusted proxy.
            y_ref_adjusted = y_ref_slice.copy()
        m_ref = None
        slope_sd_ref = None
    ...
    for r in range(1, N_OUTER_RESAMPLES + 1):
        ...
        df_r = d_model.sample(n=take_n, replace=OUTER_RESAMPLE_REPLACE, random_state=seed_r)
        try:
            gam_r = fit_gam(
...
def main():
    ...
    print(f"[cfg] slice_vars_raw={SLICE_VARS_RAW if SLICE_VARS_RAW else 'PRIMARY_ONLY'}")
    ...
    pd.DataFrame([{
        ...
        "slice_vars_raw": SLICE_VARS_RAW if SLICE_VARS_RAW else "PRIMARY_ONLY",
