...
AXIS_LABEL_EXPR <- list(
  ...
  "Pmean" = quote("Positive End-Expiratory Pressure" ~ "(cmH"[2] * "O)"),
...
X_AXIS_RULES <- list(
  ...
  "TEMP" = list(lims = c(34, 37.5), breaks = seq(34, 37, 1)),
...
FIO2_SMOOTH_SP <- suppressWarnings(as.numeric(Sys.getenv("INTRA5_FIO2_SMOOTH_SP", "0.85")))
if (!is.finite(FIO2_SMOOTH_SP)) FIO2_SMOOTH_SP <- 0.85
...
OUT_FIG_BASE <- Sys.getenv(
  "INTRA5_OUT_FIG_BASE",
  file.path(PROJECT_ROOT, "fig_output", "R_intraop5_slice_only_ppt_v2_4_1_2026_multismooth_6cm_20260616")
)
